...
class ConsoleTools(private val items: MutableList<Library>) {
    ...
    inline private fun filterBooks(list: List<Any>): List<Library> {
        return list.filter { it is Book } as List<Library>
    }

    inline private fun filterPappers(list: List<Any>): List<Library> {
        return list.filter { it is Papper } as List<Library>
    }

    inline private fun filterDisks(list: List<Any>): List<Library> {
        return list.filter { it is Disk } as List<Library>
    }
    ...
    fun showMenu() {
        ...
        while (true) {
            ...
            when (task) {
                1 -> showList(filterBooks(items))
                2 -> showList(filterPappers(items))
                3 -> showList(filterDisks(items))
                4 -> showShops()
                0 -> return
    ...
    private fun showShops() {
        ...
        while (true) {
            ...
            val task = readlnOrNull()?.toIntOrNull()
            when (task) {
                1 -> product = manager.buyBook(bookShop)
                2 -> product = manager.buyPapper(paperShop)
                3 -> product = manager.buyDisk(diskShop)
                0 -> return
                else -> {
